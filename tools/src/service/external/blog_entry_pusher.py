...
def push_hatena_blog_entry(blog_config, dir_path: str, md_file_name: str, title: str, category: str,
    ...
    if blog_entry_id is None:
        return execute_post_hatena_entry_register_api(blog_config, title, category, content)
    else:
        return execute_put_hatena_entry_update_api(blog_config, blog_entry_id, title, category, content)
