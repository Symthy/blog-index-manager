...
def resolve_backup_entry_dir_path(doc_entry_id: str):
    return f'{BACKUP_DIR_PATH}{doc_entry_id}/'


def remove_backup_doc_entries(doc_entries: DocEntries):
    for entry in doc_entries.entry_list:
        __remove_backup_doc_entries(entry)


def __remove_backup_doc_entries(doc_entry: DocEntry):
    target_dir_path = f'{BACKUP_DIR_PATH}{doc_entry.id}'
    if is_exist_dir(target_dir_path):
        delete_dir(target_dir_path)


def sava_backup_doc_entry(doc_entry: DocEntry):
    entry_backup_dir_path = resolve_backup_entry_dir_path(doc_entry.id)
...
def retrieve_backup_doc_entry(doc_entry_id: str):
    backup_entry_dir_path = resolve_backup_entry_dir_path(doc_entry_id)
